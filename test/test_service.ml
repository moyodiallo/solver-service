    ...
    Solver_service_api.Worker.Solve_request.
      {
        opam_repository_commits =
          ["github.com/ocaml/opam_repository","95d27ad970057f68179577594813dc1828324a2f"];
        root_pkgs = [];
        pinned_pkgs = [];
        platforms = [];
      }
...
let test_e2e _sw () =
  ...
  let* commits = Mock_opam_repo.commit in
  let os_id = "testOS" in
  let* vars =
  ...
  Alcotest.(check solver_response)
    "Same solve reponse"
    (Ok [ { id = os_id; packages = [ "lwt.5.5.0"; "yaml.3.0.0" ]; commits } ])
    response
