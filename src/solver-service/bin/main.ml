...
let main () commits address =
  match (commits, address) with
  | None, Some address ->
  ...
  | Some commits_str, _ -> Solver.main (Remote_commit.list_of_string_or_fail commits_str)
...
let worker_commits =
  ...
  @@ Arg.info ~doc:"The hash commits of the worker." ~docv:"COMMITS" [ "worker" ]
